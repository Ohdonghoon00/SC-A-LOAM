		...
		while ( !odometryBuf.empty() && !fullResBuf.empty() )
        {
            ...
            if( ! gtSAMgraphMade /* prior node */) {
                ...
                mtxPosegraph.lock();
                {
                    ...
                    // std::cout << curr_node_idx << std::endl;
                    // gps factor 
                    if(hasGPSforThisKF) {
            ...
            // if want to print the current graph, use 
            // gtSAMgraph.print("\nFactor Graph:\n");
...
int main(int argc, char **argv)
{
	...
	nh.param<double>("sc_dist_thres", scDistThres, 0.13);  
	nh.param<double>("sc_max_radius", scMaximumRadius, 30.0); // 80 is recommended for outdoor, and lower (ex, 20, 40) values are recommended for indoor 
